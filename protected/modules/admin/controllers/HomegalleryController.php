...
class HomegalleryController extends Controller
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['HomeGallery']))
		{
                    $model->attributes=$_POST['HomeGallery'];
                    
                    $attachments = CUploadedFile::getInstancesByName('img');
                    if(isset($attachments) && count($attachments) > 0)
                    {
                        $model->path = '/uploads/gallery/default.jpg';
                    }
                    else
                    {
                        $model->path = '';
                    ...
                    if($model->save())
                    {
                        ...
                        if(isset($attachments) && count($attachments) > 0)
                        {
                            ...
                            if(!is_dir(Yii::getPathOfAlias('webroot').'/uploads/gallery'))
                            {
                                mkdir(Yii::getPathOfAlias('webroot').'/uploads/gallery');
                            }
                            ...
                            { 
                                ...
                                if($doc->saveAs(Yii::getPathOfAlias('webroot').$path))
                                {
                                    ...
                                    {
                                        HomeGallery::model()->updateByPk($model->id, array('path'=>$path));
                                    }
                                }    
                            }
                        }
	...
	*/
	...
	{
		$dataProvider=new CActiveDataProvider('HomeGallery');
	...
	*/
	...
	{
		$model=new HomeGallery('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['HomeGallery']))
			$model->attributes=$_GET['HomeGallery'];
	...
	* @return HomeGallery the loaded model
	* @throws CHttpException
	*/
	public function loadModel($id)
	{
		$model=HomeGallery::model()->findByPk($id);
		if($model===null)
	...
	* @param HomeGallery $model the model to be validated
	*/
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='home-gallery-form')
		{
