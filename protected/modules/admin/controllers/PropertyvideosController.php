...
class PropertyvideosController extends Controller
{
	...
	*/
	public function actionCreate()
	{
            ...
            $model=new PropertyVideos;

            if(isset($_POST['PropertyVideos']))
            {
                $model->attributes=$_POST['PropertyVideos'];
                $model->property_id = $property_id;
                
                /*
                $attachments = CUploadedFile::getInstancesByName('video');
                ...
                */
                ...
                if($model->save())
                {
                    ...
                    if(isset($attachments) && count($attachments) > 0)
                    {
                        ...
                        { 
                            $file_name = $model->id."_videos.".$doc->getExtensionName();
                            $path = '/uploads/property/'.$file_name;

                            if($doc->saveAs(Yii::getPathOfAlias('webroot').$path))
                            {
                                PropertyVideos::model()->updateByPk($model->id, array('path'=>$path));
                            }    
                        }
                    }
                    */
	...
	*/
	...
	{
		$dataProvider=new CActiveDataProvider('PropertyVideos');
	...
	*/
	public function actionAdmin()
	{
            ...
            $model=new PropertyVideos('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['PropertyVideos']))
                    $model->attributes=$_GET['PropertyVideos'];
	...
	* @return PropertyVideos the loaded model
	* @throws CHttpException
	*/
	public function loadModel($id)
	{
		$model=PropertyVideos::model()->findByPk($id);
		if($model===null)
	...
	* @param PropertyVideos $model the model to be validated
	*/
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='property-videos-form')
		{
